...
str8 = "Python"
spisok12 = list(str8)
print(f"Максимальный символ в списке равен \"{max(spisok12)}\", минимальный символ в списке равен \"{min(spisok12)}\"")
#print(sum(spisok8)) - функция sum() не может складывать строки и числа вместе.
...
cities1 = ['Москва', 'Санкт-Петербург', 'Казань', 'Нижний Новгород', 'Екатеринбург']
# Каждый второй элемент, начиная с первого
print(cities1[1::2])
# Обратный порядок
print(cities1[::-1])
# Каждый второй элемент с конца, начиная с предпоследнего
print(cities1[-2::-2])
...
print([1, 2, 3] == [1, 2, 3])
print([10, 5, 3] > [5, 10, 3])
#print([1, 2, 3] > [1, 2, "abc"]) - возвращается ошибка '>' not supported between instances of 'int' and 'str' - В процессе сравнения происходит попытка сравнить целое число (int) со строкой (str), что недопустимо

chars = list("Python")
print(max(chars))
print(min(chars))
#print(sum(chars[:])) - unsupported operand type(s) for +: 'int' and 'str'. нельзя напрямую складывать строковые значения.
...
cities.sort()
sort_cities = sorted(cities) # - sort изменяю исходный список,новый список по содержанию равен исходному
